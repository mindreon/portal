...
6. （可选）把第一批管理员的企业邮箱写进 `AUTH_ADMIN_EMAILS`。不填也没关系：第一个飞书登录的人会自动成为管理员，然后在网页「权限管理」里给其他人勾模块。

飞书开放平台只能控制「谁能打开这个应用」，**不能**按模块发菜单。合同、发票各给谁看，在系统里的「权限管理」页勾选。
...
日常请让管理员登录后，点左下角 **权限管理**，给每个同事勾选「合同」「发票」。勾选立刻生效，对方刷新页面就能看到新菜单。同事必须先用飞书登录一次，才会出现在名单里。

| 能力 | 怎么得到 |
| --- | --- |
| 打开「权限管理」页 | 角色是管理员。第一个飞书登录的人自动是管理员；之后可在页面上把「可管理权限」勾给别人 |
| 看见某个业务模块 | 管理员在权限页给对应模块打勾。新同事默认只有发票 |

侧栏和工作台会藏掉没勾选的模块；有人把网址存了收藏夹，打开会看到 403，接口同样拒绝。`.env` 里的 `AUTH_ADMIN_EMAILS` 只是可选的引导名单，已经在系统里的人不会被它改回去。
...
| GET | `/api/v1/users` | 人员列表（仅管理员） |
| PATCH | `/api/v1/users/{id}` | 勾选模块 / 是否管理员（仅管理员） |
